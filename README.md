...
Log in to the Sauce Demo application.
Verify that the number of products displayed matches the expected amount.
Verify that each product's description is accurate and matches the expected descriptions.
Expected Result:
...
Verify that the cart icon displays the correct number of added products.
Verify that the "Remove" button is displayed for each product added to the cart.
Expected Result:
...
Click on "Finish" to complete the purchase.
...
Log in with valid credentials.
Navigate to the Products page.
Apply a filter by category (e.g., "T-Shirts").
